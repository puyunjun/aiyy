...
CREATE TABLE IF NOT EXISTS `dp_user_group_privilege`(
   ...
   `allow_priview_list` TINYINT(1) UNSIGNED NOT NULL DEFAULT 1 COMMENT '是否允许进入伴游详情,1=>允许,0=>禁止',
   `allow_priview_photo` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否允许查看照片,1=>允许,0=>禁止',
   `allow_priview_video` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否允许查看视频,1=>允许,0=>禁止',
   `allow_chat` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否允许聊天,1=>允许,0=>禁止',
   `allow_insurance` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否享受保险,1=>是,0=>不是',
   `allow_recommend` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否享受客服推荐,1=>是,0=>不是',
   `allow_videoconferencing` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否允许真人视频,1=>允许,0=>禁止',
   `allow_escort_recommend` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否享受高级伴游推荐，1=>是,0=>不是',
   `allow_date` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否允许真人见面,1=>允许,0=>禁止',
