...
$('body').css({"background":"url('/static/home/css/wapcssjsimg/images/login.jpg') no-repeat top #121820", "background-size":"100% auto"});
...
function sub_fun(){
    ...
    setTimeout(function(){
        $.ajax({
            url:'http://'+window.location.host+'/user/Signin/index',
            type:'post',
            dataType:'JSON',
            data:{username:phone,verify_code:verify_code,password:$('#password').val()},
            success:function(re){
                $.hideLoading();
                if(re.status === false){
                    layer.msg(re.msg)
...
//window.location.href = 'http://'+window.location.host+'/api/Login/index';
